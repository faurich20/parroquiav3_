            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  console.log('💳 [FRONTEND] Validación exitosa, enviando a backend...');

                  const payload = {
                    // Mantener datos existentes
                    horarioid: current.horarioid,
                    ...
                    pago_descripcion: `Pago por reserva litúrgica - ${current?.persona_nombre || 'N/A'}`,
                    pago_fecha: new Date().toISOString()
                  };

                  const result = await updateItem(current.reservaid || current.id, payload);
  ...
  const columns = useMemo(() => ([
    ...
    },
    {
      ...
      render: (r) => {
        ...
        console.log('Row:', r.id, 'Estado Original:', r.pago_estado, 'Estado Normalizado:', estado);
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            {(estado === 'pendiente' || !r.pago_estado) && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  const data = prepareEditData(r);
                  console.log('💳 [TABLE] Opening payment modal for:', data);
                  setCurrent(data);
                  setPaymentData({
                    pago_medio: '',
                    pago_monto: '',
